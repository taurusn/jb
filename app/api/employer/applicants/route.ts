...
import { handleGetUnrequestedEmployees } from '@/backend/controllers/employee.controller';
 ...
 *     description: Get paginated list of job applicants with optional filters
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - name: page
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Unrequested applicants query params:', query);
    ...
    // Get unrequested applicants only
    const result = await handleGetUnrequestedEmployees(employerProfile.id, query);

    console.log('Unrequested applicants result:', result);
